...
QPUPControlLoopReal::QPUPControlLoopReal(const ros::NodeHandle &nh) : QPUPControlLoop("qpup_control_loop_real", nh) {
  control_freq_ = qpup_utils::getParam(private_nh_, name_, "control_frequency", DEFAULT_CONTROL_FREQUENCY);
  controller_watchdog_timeout_ =
      qpup_utils::getParam(private_nh_, name_, "controllers_watchdog_timeout", DEFAULT_CONTROLLER_WATCHDOG_TIMEOUT);
  if (!this->init()) {
