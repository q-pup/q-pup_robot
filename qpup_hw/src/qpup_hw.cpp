...
void QPUPHW::read(const ros::Time & /*time*/,
...
void QPUPHW::write(const ros::Time & /*time*/,
                   const ros::Duration & /*period*/) {
...
void QPUPHW::registerStateInterfacesAndTransmissions(
  ...
  hardware_interface::JointStateHandle joint_state_handle(
      ...
      &actuator_joint_states_[joint_name].joint_velocity,
      &actuator_joint_states_[joint_name].joint_effort);
  joint_state_interface_.registerHandle(joint_state_handle);

  // Wrap Actuators States
  actuator_state_data_[joint_name].position.push_back(
      &actuator_joint_states_[joint_name].actuator_position);
  actuator_state_data_[joint_name].velocity.push_back(
      &actuator_joint_states_[joint_name].actuator_velocity);
  actuator_state_data_[joint_name].effort.push_back(
      &actuator_joint_states_[joint_name].actuator_effort);

  // Wrap Joint States
  joint_state_data_[joint_name].position.push_back(
      &actuator_joint_states_[joint_name].joint_position);
  joint_state_data_[joint_name].velocity.push_back(
      &actuator_joint_states_[joint_name].joint_velocity);
  joint_state_data_[joint_name].effort.push_back(
      &actuator_joint_states_[joint_name].joint_effort);
  ...
  transmission_interface::ActuatorToJointStateHandle
      actuator_to_joint_state_handle(
          joint_name, &joint_transmissions_.find(joint_name)->second,
          actuator_state_data_[joint_name], joint_state_data_[joint_name]);
...
void QPUPHW::registerCommandInterfacesAndTransmissions(
    const std::string &joint_name) {
  // Register JointHandle associated with a JointStateHandle to command
  // interfaces
  hardware_interface::JointHandle joint_command_handle(
  ...
  actuator_command_data_[joint_name].position.push_back(
      &actuator_joint_commands_[joint_name].actuator_data);
  actuator_command_data_[joint_name].velocity.push_back(
      &actuator_joint_commands_[joint_name].actuator_data);
  actuator_command_data_[joint_name].effort.push_back(
      &actuator_joint_commands_[joint_name].actuator_data);

  // Wrap Joint Commands
  joint_command_data_[joint_name].position.push_back(
      &actuator_joint_commands_[joint_name].joint_data);
  joint_command_data_[joint_name].velocity.push_back(
      &actuator_joint_commands_[joint_name].joint_data);
  joint_command_data_[joint_name].effort.push_back(
      &actuator_joint_commands_[joint_name].joint_data);
  ...
  transmission_interface::JointToActuatorPositionHandle
      joint_to_actuator_position_handle(
          joint_name, &joint_transmissions_.find(joint_name)->second,
          actuator_command_data_[joint_name], joint_command_data_[joint_name]);
  ...
  transmission_interface::JointToActuatorVelocityHandle
      joint_to_actuator_velocity_handle(
          joint_name, &joint_transmissions_.find(joint_name)->second,
          actuator_command_data_[joint_name], joint_command_data_[joint_name]);
  joint_to_actuator_velocity_interface_.registerHandle(
      joint_to_actuator_velocity_handle);
  ...
  transmission_interface::JointToActuatorEffortHandle
      joint_to_actuator_effort_handle(
          joint_name, &joint_transmissions_.find(joint_name)->second,
          actuator_command_data_[joint_name], joint_command_data_[joint_name]);
  joint_to_actuator_effort_interface_.registerHandle(
      joint_to_actuator_effort_handle);
}
